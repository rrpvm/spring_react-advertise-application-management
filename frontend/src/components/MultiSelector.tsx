...
export const MultiSelector: React.FC<IMultiselectable> = ({ uniqueStrings, alreadySelected, onItemClick }): JSX.Element => {
    ...
    function add(str: string): void {
        ...
        selectChangedHandler(mutableSelected);//setSelected
    ...
    const selectChangedHandler = (arr_quick: string[]) => {//quick - dont wait for state change (send mutable variable)
        onItemClick(arr_quick);
    }
    useEffect(() => {
            ...
            <>
                {
                    selectedItems?.map(item => {
                        return (
                            <div style={{ display: "flex", background: "rgb(230, 230, 230)", alignItems: "center", padding: "5px 0px" }} key={item}>
                                <span style={{ fontSize: "1em", padding: "0px 0.2em" }}>{item}</span>
                                <div onClick={() => { remove(item) }} style={{ padding: "0px 0.5em", display: "flex", alignItems: "center", borderRadius: "2px", height: "100%", position: "relative", justifyContent: "center", alignContent: "center" }} className="hoverable">
